...
namespace :deploy do
  ...
  task :sync_config do
    # this will sync the config file on your local machine with that on your production server
    # notice that the domain variable is used (this should be the ip or domain of your app)
    # we set this variable way on top
    # also note that this puts it in the shared folder, the symlink task below will like it from the release folder
    
    # make sure you install rsync on the server
    system "rsync -vr --exclude='*~' config #{user}@#{domain}:#{shared_path}/"
  ...
  desc "Symlink shared config folder on each release."
  task :symlink_shared do
    # creates symlink for the config folder
 ...
 after 'deploy:update_code', 'deploy:sync_config', 'deploy:symlink_shared', 'deploy:restart'
...
You should do this as root, just in case.  On your production server (we did this for gem earlier):
