...
We installed MySQL gem above, but we still need to install MySQL itself.

bq. apt-get install mysql-server

h4. Create Database

Once the installation is complete, let's log into mysql and create the database we need for our application.
...
On your local machine (you don't need to install anything on the production server):
...
OK - finally, with that leg work done, it's time to set up Capistrano itself.  This is set up on your local machine only.

From your tutorial project....
